...
int main(){
  ...
  for(int i=0; i<100000000; ++i){
    ...
    if(i%10000==0) graphpv->SetPoint(i/10000, vc+ve+vh+vr, p);
        if(i%100==0) graphts->SetPoint(i/100, 5./2.*R*M*log(p*pow(vc+ve+vh+vr,7./5.))+.43,p*(vc+ve+vh+vr)/R/M);//M*7./2.*R*log(tc)-M*R*log(p)-0.6, tc);
        if(t>T) break;
      ...
      yaxis->SetTitle("p (Pa)");
      TAxis* xaxisT = graphts->GetXaxis();
      TAxis* yaxisT = graphts->GetYaxis();
      xaxisT->SetTitle("S (J/K)");
      yaxisT->SetTitle("T (K)");

    //xaxisT->SetLimits(.37,.46);
    //yaxisT->SetRangeUser(370,505);
      ...
      cout<<W<<endl;
      cout<<graphpv->Integral()<<endl;
      cout<<graphts->Integral()<<endl;
      cout<<PV.Integral()<<endl;
      cout<<TS.Integral()<<endl;
